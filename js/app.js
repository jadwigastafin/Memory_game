$(function() { //Funkcja samowywołująca zabezpiecza skrypt, treśc nie dostępna na zewnatrz
    ...
    let clickedTiles = []; //kliknięte kafelki max2
    ...
    function tileClicked(element) {
        ...
        if(canGet) {  //canGet = true
            ...
            if (!clickedTiles[0] || (clickedTiles[0].data('index') != element.data('index'))){ //jeżeli jeszcze nie pobraliśmy 1 elementu lub jego indeksu nie ma w pobranych
            ...
            if (clickedTiles.length == 2) {  //clickedTiles zawiera numery aktualnie klkiniętych kafelków
                ...
                if (clickedTiles[0].data('cardType') == clickedTiles[1].data('cardType')) { //sprawdzamy czy typ obu kafelków jest taki sam
                    setTimeout(function() {
